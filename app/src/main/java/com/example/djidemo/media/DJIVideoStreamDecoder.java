package com.example.djidemo.media;
...
public class DJIVideoStreamDecoder implements NativeHelper.NativeDataListener {
    ...
    private void initCodec() {
        ...
        if (surface == null) {
            ...
            // The surface is null, which means that the yuv data is needed, so the color format should  COLOR_Format24bitBGR888    COLOR_FormatYUV420SemiPlanar COLOR_FormatYUV420Planar
            // be set to YUV420.
            format.setInteger(MediaFormat.KEY_COLOR_FORMAT, MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar);
//            MediaCodecInfo a = selectCodec("video/avc");
//            printColorFormat(a, "video/avc");
//            saveLogs("nyb: in djivideostream", "MediaFormat.KEY_COLOR_FORMAT is " + MediaFormat.KEY_COLOR_FORMAT);
        } else {
    ...
    private static void printColorFormat(MediaCodecInfo codecInfo, String mimeType) {
        ...
        for (int i = 0; i < capabilities.colorFormats.length; i++) {
            ...
            saveLogs("nyb: in in djivideostream color format:", String.valueOf(colorFormat));
            switch (colorFormat) {
